...
// Canonical client configuration. Keep this defined before any auth bootstrap
// path can call ensureClient(); expose it for the shared call receiver as well.
const SPIKE_CONFIG = Object.freeze({
  supabaseUrl: SUPABASE_URL,
  supabaseKey: SUPABASE_KEY,
  authRedirectUrl: AUTH_REDIRECT_URL,
  passwordResetRedirectUrl: PASSWORD_RESET_REDIRECT_URL
});
window.SPIKE_CONFIG = SPIKE_CONFIG;
window.SPIKE_SUPABASE_CONFIG = Object.freeze({
  url: SUPABASE_URL,
...
// Never query profiles until Supabase has an authenticated session.
// A user object without a session would send this request as anon, which is
// intentionally denied access to profiles by the production RLS/grants.
const { data: sessionData, error: sessionError } = await db.auth.getSession();
if (sessionError) throw sessionError;
if (!sessionData?.session?.user?.id || sessionData.session.user.id !== user.id) {
  throw new Error("Your SPIKE session was not established. Please sign in again.");
}
...
if (data) return data;
throw new Error("Your SPIKE profile is still being created. Please wait a moment and try again.");
}
...
throw new Error("Your SPIKE profile could not be loaded yet. Please try signing in again.");
}
...
if (!data.session) {
  // Do not fabricate a session from data.user. That causes profile requests to
  // run with the anon role and correctly fail with 42501.
  const { data: current, error: currentError } = await db.auth.getSession();
  if (currentError) throw currentError;
  if (!current?.session) throw new Error("SPIKE signed in the account but could not establish the browser session. Please try again.");
}
const completed = await resolveSessionOnce(data.session || (await db.auth.getSession()).data.session, "signin");
